...
I_SIGNAL_I_PDU PDU_1;

// std::multimap<std::string, std::multimap<std::string, int> > Map;
// 先序遍历，读出所有并列的 I-SIGNAL-I-PDU元素
bool XMLDocument::generateMap3(const XMLElement* element) const
{
    ...
    while (tempElement != NULL || !elementStack.empty())
    {
        ...
        {
            decideClassType(tempElement->Name());
            
            printf("\n%s---", tempElement->Name());
            const XMLAttribute* attribute = tempElement->FirstAttribute();
            ...
            {
                printf("文本: %s", tempElement->GetText());
            }
